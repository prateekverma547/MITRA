...
> looks arbitrary.** Nearly every constant, threshold and prompt shape in this
> repo was measured, and the numbers are recorded there. This file holds the
> rules; that file holds the evidence.
>
> | | |
> | --- | --- |
> | [README.md](README.md) | run it locally |
> | [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) | how it fits together, and which layer owns which symptom |
> | [docs/BRAIN.md](docs/BRAIN.md) | the interview brain and its invariants |
> | [docs/DECISIONS.md](docs/DECISIONS.md) | every decision with its measurement |
> | [docs/OPERATIONS.md](docs/OPERATIONS.md) | running the deployed system |
>
> The milestone plan below is now **history rather than instruction**, all six
> are delivered. It is kept because the Definition of Done for each records what
> "working" was taken to mean, which is often the thing a later change breaks.
...
- **Proactive silence handling.** A silence timer measured from the end of the bot's last utterance with no candidate speech started, driving a three-stage escalation ladder (all timings configurable, in `bot/silence.py`):
  ...
  - **The ladder holds when the room is empty** (`bot/presence.py`). Daily reports participants joining and leaving; nothing listened, so the bot nudged empty rooms and then closed the session blaming a silence that was really a dropped call. Presence turns that guess into a fact: no nudges into an empty room, and the ending says the call dropped. Disconnects flow into `ConversationHealth`, so a dropped call reads as a bad connection rather than an uncooperative candidate. A second participant is also visible now, recorded and mentioned once, never policed.
  - **Patience scales with the question, and only the first rung moves.** A uniform twenty seconds treats "what are you working on?" and "tell me about a decision you regret" as the same question. The second is someone searching their memory, and prompting into that is the interruption problem arriving slowly. The brain supplies the multiplier because it is the only thing that knows what it just asked: 0.75x in the opening and closing, 1.6x once probing inside a competency. Later rungs are about the channel and the session, not about thinking, so they stay put (they shift only enough to stay clear of a stretched first rung).
  - This is the one piece that **cannot be verified by a scripted conversation**, there is no real dead air in text mode. `scripts/silence_walkthrough.py` prints what the ladder would do in each scenario, but whether the timings *feel* right is a judgement to be made by ear.
...
- **The interview speaks the field's language, borrowed and never invented** (`InterviewRegister`, `blueprint/generate.py`). A generic interview sounds like it was written by someone who has never done the job: a business analyst should hear "BRD", "UAT" and "sign-off", not "process improvement activities".
  - **Every term is verified against the JD and the CV in code**, the same shape as quote verification in the feedback report and for the same reason. Jargon used wrongly is far worse than plain language, because a specialist hears one misused term and stops trusting the interview. Borrowing only what the documents already say makes that impossible rather than unlikely.
  - Trailing-`s` tolerance only, so "BRD" matches a JD that says "BRDs". Anything looser stops being verification.
  - Capped at 12 terms: the live context is small on purpose, and an interviewer reciting thirty terms sounds like it is showing off rather than listening. An empty register is a fine outcome and means plain language.
  - The prompt carries the caution as well as the words: plain language is always safe, and if the candidate uses a term the interviewer was not given, follow their lead.
  - Blueprint generation now receives `jd_text` as well as the CV; it could not see the employer's own vocabulary before.
...
- **Sectioned interview brain**, state machine over the blueprint:
  ...
  - **Repair handling** (`bot/brain/repair.py`). Three things a candidate means when a question misfires, and three different answers. Conflating them is what makes a voice bot feel stupid.
    - *"sorry, what?"* → they did not hear it. **Say the same question again.** Rephrasing here is actively confusing; the words were never the problem.
    - *"what do you mean?"* → they heard it but it did not land. **Say it in simpler words.** Repeating verbatim is useless.
    - *"I don't know"* → **that is an answer.** Not confusion, not a refusal. Treating it as either punishes candour or loops forever.
    - Deterministic and whole-utterance like refusal detection, and for the same reason: reading a real answer as a repair request throws the answer away.
    - A repair **does not spend a turn**, or the depth ramp advances on nothing. Two attempts at a question is the cap; a third is an interrogation, and recording that the ground was never covered is more honest.
    - The prompt block removes the alternative rather than discouraging it, per the contradiction lesson. Measured need: a live run showed the interviewer answer *"sorry, you cut out there"* by dropping the question and asking a different one, losing the answer and its coverage.
  - **Refusal handling.** A candidate declining is disengaged, uncomfortable, or done; the bot is not entitled to an answer and must never interrogate. Detection is deliberately conservative and deterministic (`bot/brain/refusal.py`), matched against the whole utterance, so "no" is a refusal but "no, that's not how it went, what we did was…" is an answer. Treating a real answer as a refusal is the worse error.
    ...
    - **Withdrawal is not refusal, and the call actually ends** (`bot/brain/withdrawal.py`). "I'd rather not answer that" refuses a question; "I don't want to continue the interview" is somebody leaving. The second used to end nothing until four consecutive refusals had accumulated, so a candidate who said plainly that they wanted to stop was asked several more questions first.
    - **Detection is semantic, because pattern matching provably cannot do this.** The interviewer model is given an `end_interview` tool (`bot/tools.py`) and calls it itself. Measured against sixteen natural ways of asking to stop, the pattern matcher caught **zero**: "can we wrap this up", "this isn't for me", "I need to jump off", "bas, ab band karo". The model caught **10 of 10** of the same set. A phrase list only ever contains what somebody thought of in advance.
      - This does not put a judgement on the critical path. It is the same inference call the interviewer was already making, plus a few tokens of schema: no second model, no second request, nothing to wait for. Compliance is structural because the brain acts on the tool call rather than on the model following prose.
      - **The two mechanisms corroborate each other**, because each is strong where the other is weak. The model produced 2 false positives out of 6, including calling the tool on *"we had to end that project early because of budget"*; the patterns produced none but only catch anticipated wording. So: recall from the model, precision from the patterns. Both agree → the interview ends at once. Only the model → they are asked once, which costs a sentence if it was wrong and an interview if it was not.
      - Re-measure with `scripts/tool_call_check.py` after any edit to the tool description.
    - **Intent is searched for inside the utterance, not matched as the whole of it.** The first version used whole-utterance matching and failed live against everything a person actually says: *"I don't want to continue this interview. Just end this interview."*, *"Just end in the interview."* (as transcribed), *"No, just end the interview."* Four requests in under a minute, none detected, and the interviewer answered the third by asking a fresh question about learning new business domains. Every pattern names the interview, call or session, so *"we had to end the project early"* is still an answer.
    - **Two tiers, because being asked twice is the insult.** An explicit instruction (*"just end this interview"*) closes immediately. Only a softer signal (*"I'm done"*) gets one offer, and repeating it in any form counts as accepting. Nobody is asked a third time.
    - A candidate already leaving must not re-trigger the withdrawal path: it returned early, so the closing check never ran and the goodbye repeated without the interview ever registering as finished.
    - The goodbye after a withdrawal does not invite questions. They asked to leave, and fishing for more conversation is not respecting that.
    - **The interview ends when the interviewer has finished closing it**, never when the candidate next speaks. The closing section advanced on candidate turns like every other section, so after a goodbye nobody replied to, `turns_spent` never reached its ceiling, `is_finished` stayed False, and the call sat open with both parties in it. A closing turn containing a question is inviting a last one and waits; one without is a goodbye and ends. Capped at two interviewer turns.
    - **Nothing ended the call before this.** `is_finished` existed and only the text harness read it, so even a normally completed interview sat there after the goodbye until the candidate hung up or the silence ladder closed it two minutes later *as an abandonment*. `bot/ending.py` ends on `BotStoppedSpeaking` once the brain is finished, so the goodbye is heard first.
    - **It is an observer, and that is not a style choice.** The first attempt put the check in `BrainDirector`, which sits before `transport.output()` in the pipeline and therefore never sees the frames that transport emits. It shipped, and live the bot said goodbye and the call stayed open. A processor cannot react to anything produced downstream of it; observers see every frame wherever they sit, which is why the silence ladder has always worked.
    - **Observers must not be plain dataclasses.** Pipecat keeps them in a set, and `@dataclass` generates `__eq__`, which removes the default hash: registration dies with `unhashable type` and the call never ends. Use `@dataclass(eq=False)` or a plain class. Both of these were found only by running a real pipeline through `pipecat.tests.utils.run_test`; every direct-call unit test passed against the broken versions.
    - Skipped competencies say which ending caused them. A candidate who chose to leave is never written up as one who ran out of time.
    - **Found only by a scripted conversation.** The detector missed "Sorry, I do not want to continue the interview" because "sorry" is not a filler, and the interviewer responded by asking for feedback about the interview process and carrying on. Every unit test passed; they had all used the phrase without the apology.
