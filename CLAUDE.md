...
- **Sectioned interview brain** — state machine over the blueprint:
  ...
  - **Refusal handling.** A candidate declining is disengaged, uncomfortable, or done; the bot is not entitled to an answer and must never interrogate. Detection is deliberately conservative and deterministic (`bot/brain/refusal.py`) — matched against the whole utterance, so "no" is a refusal but "no, that's not how it went, what we did was…" is an answer. Treating a real answer as a refusal is the worse error.
    ...
    - **Detected to a stricter standard than anything else here, because the consequence is terminal.** A missed withdrawal costs one more question they can repeat; a false one ends an interview and cannot be undone. Whole-utterance, explicit phrases, never keywords. "I don't want to talk about my last employer" must never end an interview.
    - **Offered, never assumed.** One sentence: that is completely fine, would you like to end here or carry on. A misdetection then costs a sentence rather than an interview. An ambiguous reply is taken as carrying on; pressing somebody about leaving is worse than one wasted turn.
